...
# Configuration
SHEET_NAME = "stock-sheet-gurjyot"  # Name of Google Sheet
...
def get_stock_data(ticker):
    ...
    try:
        ...
        if len(aggs) >= 2:
            ...
            day_change_pct = 0.0
        logging.info(f"Fetched data for {ticker}: price={current_price}, "
...
def get_ai_analysis(ticker, current_price, day_change_pct, buy_price):
    ...
    - NOTE: [If buy more: According to my buy price, when should I add more?]
            [if sell: What price to sell at?] <keep it short, strong prediction
            and accurate. >
    """
    try:
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a leading financial analyst "
                                              "providing technical analysis and "
                                              "recommendations on stocks and crypto "
                                              "Your task is to analyze the stock for "
                                              "promising profit."},
    ...
    for i in range(0, len(records), batch_size):
        ...
        for record in batch:
            ...
            try:
                ...
                profit_loss = calculate_profit_loss(buy_price, current_price,
                                                    quantity)
                ai_analysis = get_ai_analysis(ticker, current_price,
                                             day_change_pct, buy_price)
